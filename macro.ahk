...
tooltip, Made By catdascripter, %TooltipX%, %Tooltip1%, 1
...
; Failsafe to stop the shaking process if the limit is reached
ClickShakeFailsafe:
    ClickFailsafeCount++
...
; Main mode for click shaking
ClickShakeMode:
    ; Display initial tooltips
...
ClickShakeModeRedo:
    ; Exit if forced reset is triggered
    if (ForceReset)
    {
        tooltip,,,%TooltipX%,11
        tooltip,,,%TooltipX%,12
    ...
    {
        ; Scan for clickable pixels in the shaking area
...
if (ErrorLevel == 0)
	...
	if (FishX < MaxLeftBar)
		...
		if (MaxLeftToggle == false)
			...
			tooltip, Counter Delay: None, %TooltipX%, %T